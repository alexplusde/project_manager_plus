...
rex_sql_table::get(rex::getTable('project_manager_plus_logs'))
    ->ensurePrimaryIdColumn()
    ->ensureColumn(new rex_sql_column('domain_id', 'int(10) unsigned', false))
    ->ensureForeignKey(new rex_sql_foreign_key('domain_id', rex::getTable('project_manager_plus_domain'), ['domain_id' => 'id']))
...
$cronjob->setQuery('SELECT id FROM '.rex::getTable('cronjob'). ' WHERE type LIKE "rex_cronjob_project_manager_plus_data"');
...
$cronjob = rex_sql::factory();
$cronjob->setDebug(false);
$cronjob->setQuery('SELECT id FROM '.rex::getTable('cronjob'). ' WHERE type LIKE "rex_cronjob_project_manager_plus_favicon"');
...
// Create tables
rex_sql_table::get(rex::getTable('project_manager_plus_domain_hosting'))
    //->ensurePrimaryIdColumn()
    ->ensureColumn(new rex_sql_column('domain', 'varchar(255)', false, null, ''))->setPrimaryKey('domain')
    ->ensureColumn(new rex_sql_column('raw', 'longtext', true))
