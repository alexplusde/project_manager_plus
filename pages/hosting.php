...
if ($showlist) {
    ...
    $sql = 'SELECT * FROM (
          ...
          LEFT JOIN (
                        ...
                        FROM ' . rex::getTable('project_manager_plus_domain_hosting') . '
              ) as H
    ...
    $sql2 = 'SELECT * FROM  '. rex::getTable('cronjob').'
          WHERE type = "rex_cronjob_project_manager_plus_hosting"';
    $cronjob = rex_sql::factory()->getArray($sql2);
    $cronjobId = $cronjob[0]['id'];
  
    $refresh = '';
    if ($cronjobId != null) {
        $refresh = '<a href="/redaxo/index.php?page=project_manager_plus/hosting/overview#" data-cronjob="/redaxo/index.php?page=cronjob/cronjobs&func=execute&oid='.$cronjobId.'&_csrf_token='.$csrf_token.'" target="_blank" class="pull-right callCronjob"><i class="fa fa-refresh"></i> Hosting Daten aktualisieren</a>';
    }
    echo rex_view::info("Anzahl der Domains und Projekte: ".count($items) . $refresh);
  
    $list = rex_list::factory($sql, 1000);
    $list->addTableAttribute('class', 'table-striped');
    $list->setNoRowsMessage($this->i18n('project_manager_plus_hosting_domain_norows_message'));
    ...
    $list->setColumnLabel('name', $this->i18n('project_manager_plus_hosting_name'));
    $list->setColumnParams('name', ['page' => 'project_manager_plus/server/projects', 'func' => 'updateinfos', 'domain' => '###domain###']);
  
    $list->setColumnLabel('createdate_psi', $this->i18n('project_manager_plus_hosting_updatedate'));
    ...
    $list->addColumn($this->i18n('project_manager_plus_hosting_domain'), '###domain###', 3);
    //$list->setColumnParams($this->i18n('project_manager_plus_hosting_domain'), ['page' => 'project_manager_plus/server/projects', 'func' => 'updateinfos', 'domain' => '###domain###']);
    $list->setColumnFormat($this->i18n('project_manager_plus_hosting_domain'), 'custom', function ($params) {
    ...
    $list->setColumnLabel('ip', $this->i18n('project_manager_plus_hosting_ip'));
