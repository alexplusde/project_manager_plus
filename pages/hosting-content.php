...
if($domain) {
  ...
  $query = 'SELECT * FROM `rex_project_manager_plus_domain_hosting` AS H
  ...
  if (count($result) > 0) {
    ...
    if(is_array($raw)) {
      ...
      $output = '<table class="table table-striped"><thead><tr><th>'.$this->i18n('organisation').'</th><th>'.$this->i18n('isp').'</th><th>'. $this->i18n('project_manager_plus_hosting_ip').'</th><th>'.$this->i18n('validFrom').'</th><th>'.$this->i18n('validTo').'</th></tr></thead><tbody>';
