...
$csrf = rex_csrf_token::factory('project_manager_plus');
...
if ($showlist) {
    ...
    $sql = 'SELECT * FROM (
              SELECT * FROM  '. rex::getTable('project_manager_plus_domain') . ' as X ORDER BY name ASC
            ) AS D
						LEFT JOIN (
						  					...
						  					FROM ' . rex::getTable('project_manager_plus_domain_psi') . '
											 ) as PSI
    ...
    $sql2 = 'SELECT * FROM  '. rex::getTable('cronjob').'
          WHERE type = "rex_cronjob_project_manager_plus_pagespeed"';
    $cronjob = rex_sql::factory()->getArray($sql2);
    $cronjobId = $cronjob[0]['id'];
    
    $refresh = '';
    if ($cronjobId != null) {
        $refresh = '<a href="/redaxo/index.php?page=project_manager_plus/pagespeed/overview/#" data-cronjob="/redaxo/index.php?page=cronjob/cronjobs&func=execute&oid='.$cronjobId.'&_csrf_token='.$csrf_token.'" target="_blank" class="pull-right callCronjob"><i class="fa fa-refresh"></i> PageSpeed Daten aktualisieren</a>';
    }
    echo rex_view::info("Anzahl der Domains und Projekte: ".count($items) . $refresh);
    
    $list = rex_list::factory($sql, 1000);
    $list->addTableAttribute('class', 'table-striped');
    $list->setNoRowsMessage($this->i18n('project_manager_plus_pagespeed_domain_norows_message'));

    $list->setColumnFormat('id', 'Id');
    $list->addParam('page', 'project_manager_plus/server');
    ...
    $list->setColumnLabel('name', $this->i18n('project_manager_plus_pagespeed_name'));
    $list->setColumnParams('name', ['page' => 'project_manager_plus/server/projects', 'func' => 'updateinfos', 'domain' => '###domain###']);
  
    $list->setColumnLabel('createdate_psi', $this->i18n('project_manager_plus_pagespeed_updatedate'));
    ...
    $list->setColumnFormat($thIcon, 'custom', function ($params) {
        $filename = '';
        if (file_exists(rex_plugin::get('project_manager_plus', 'server')->getAssetsPath('favicon/'.$params['list']->getValue('domain').'.png'))) {
            $filename = rex_plugin::get('project_manager_plus', 'server')->getAssetsUrl('favicon/'.$params['list']->getValue('domain').'.png');
    ...
    $list->addColumn($this->i18n('project_manager_plus_pagespeed_domain'), '###domain###', 3);
    //$list->setColumnParams($this->i18n('project_manager_plus_pagespeed_domain'), ['page' => 'project_manager_plus/server/projects', 'func' => 'updateinfos', 'domain' => '###domain###']);
    $list->setColumnFormat($this->i18n('project_manager_plus_pagespeed_domain'), 'custom', function ($params) {
    ...
    $list->setColumnFormat('status_psi', 'custom', function ($params) {
        ...
        } elseif ($params['list']->getValue('status_psi') == "0") {
            return '<span class="hidden">2</span><span class="rex-icon fa-question text-warning"></span>';
        } elseif ($params['list']->getValue('status_psi') == "-1") {
            return '<span class="hidden">3</span><span class="rex-icon fa-exclamation-triangle text-danger"></span>';
        } elseif ($params['list']->getValue('status_psi') == "2") {
            return '<span class="hidden">3</span><span class="rex-icon fa-arrow-right text-danger"></span>';
        } else {
            if ($params['list']->getValue('is_ssl') == 1) {
                return '<a href="https://www.'.$params['list']->getValue('domain').'/?rex-api-call=project_manager_plus&api_key='.$params['list']->getValue('api_key').'"><span class="rex-icon fa-question"></span></a>';
            }
        
            if ($params['list']->getValue('is_ssl') == 0) {
                return '<a href="http://www.'.$params['list']->getValue('domain').'/?rex-api-call=project_manager_plus&api_key='.$params['list']->getValue('api_key').'"><span class="rex-icon fa-question"></span></a>';
            }
        }
    });
    ...
    $list->setColumnFormat("Pagespeed", 'custom', function ($params) {
        ...
        } elseif ($params['list']->getValue('psi_score_desktop') < 0.9) {
        ...
        } elseif ($params['list']->getValue('psi_score_mobile') < 0.9) {
    ...
    $content = $list->get();
    $content = str_replace('<table class="', '<table class="project_manager_plus-tablesorter ', $content);
