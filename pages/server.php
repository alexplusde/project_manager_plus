...
if ($func != '') {
    ...
    $yform->setObjectparams('main_table', rex::getTable('project_manager_plus_domain'));
    $yform->setObjectparams('form_name', 'project_manager_plus_form');
    
    $yform->setValueField('text', ['name', $this->i18n('project_manager_plus_server_name'), 'notice' => '<small>'.$this->i18n('name_info').'</small>']);
    $yform->setValidateField('empty', ['name', $this->i18n('project_manager_plus_server_no_name_defined')]);
    $yform->setValidateField('unique', ['name', $this->i18n('project_manager_plus_server_name_already_defined')]);
    
    $yform->setValueField('text', ['domain', $this->i18n('project_manager_plus_server_domain'), 'notice' => '<small>'.$this->i18n('domain_info').'</small>']);
    $yform->setValidateField('empty', ['domain', $this->i18n('project_manager_plus_server_no_domain_defined')]);
    $yform->setValidateField('unique', ['domain', $this->i18n('project_manager_plus_server_domain_already_defined')]);
    ...
    if ($func == 'delete') {
        ...
        if (!$csrf->isValid()) {
            ...
            $d = rex_sql::factory();
            $d->setQuery('delete from '.rex::getTable('project_manager_plus_logs').' where domain_id=' . $data_id);
            $e = rex_sql::factory();
            $e->setQuery('delete from '.rex::getTable('project_manager_plus_domain').' where id=' . $data_id);
...
if ($showlist) {
    ...
    $sql = 'SELECT * FROM (
            ...
            LEFT JOIN (SELECT domain_id, `raw`, createdate  FROM rex_project_manager_plus_logs WHERE id IN (SELECT MAX(id) FROM rex_project_manager_plus_logs GROUP BY domain_id)) AS L
            ON D.id = L.domain_id
            ORDER BY '.$sort.' '.$sorttype.'';

    $list = rex_list::factory($sql, 2000);
    $list->setColumnFormat('id', 'Id');
    $list->addParam('page', 'project_manager_plus/server/overview');
    
    $items = rex_sql::factory()->getArray($sql);
    
    // Cronjobcall
    $sql2 = 'SELECT * FROM  '. rex::getTable('cronjob').'
          WHERE type = "rex_cronjob_project_manager_plus_data"';
    ...
    $list->setColumnParams(rex_i18n::msg('view'), ['page' => 'project_manager_plus/server/projects', 'domain' => '###domain###'] + $csrf->getUrlParams());
